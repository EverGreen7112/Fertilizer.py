...
class Shape:
    ...
    def calculate_distance(self, original_area: float, focal_length: float) -> float:
        distance_from_rect = find_distance_from_objecet(
            focal_length, self.area, original_area)
        return distance_from_rect

    def calculate_angle(self, camera) -> float:
        distance_from_center_of_image = distance_from_center(
            self.center, camera.middle)
        angle = find_angle(distance_from_center_of_image,
...
class Rectangle(Shape):
    ...
    def calculate_angle(self, camera) -> float:
        distance_from_center = distance_from_center__rect(
            self.point1, self.point2, camera.middle)
        angle = find_angle(distance_from_center, camera.fov,
